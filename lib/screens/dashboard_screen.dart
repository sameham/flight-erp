...
/// الشاشة الرئيسية - Dashboard (متصلة بـ Supabase)
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Map<String, FinanceModel> _finances = {};
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        ...
        _finances = results[1] as Map<String, FinanceModel>;
        _refunds = results[2] as List<RefundModel>;
        _loading = false;
      });
    } catch (e) {
      setState(() {
        _loading = false;
        _error = 'تعذر الاتصال بقاعدة البيانات.\nتأكد من الإنترنت وإعدادات Supabase.';
      });
    }
  }

  // ===== الحسابات المالية من البيانات الحقيقية =====
  double get _totalProfit {
    double sum = 0;
    for (final b in _bookings) {
      if (b.status == BookingStatus.refunded ||
          b.status == BookingStatus.cancelled) continue;
      sum += _finances[b.id]?.profitMargin ?? 0;
    }
    return sum;
  }
  ...
  double get _totalDue {
    double sum = 0;
    for (final f in _finances.values) {
      sum += f.dueAmount;
    }
    return sum;
  }

  double get _totalPendingRefunds {
    double sum = 0;
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () async {
          ...
          if (saved == true && mounted) {
            _load(); // إعادة التحميل من قاعدة البيانات
            ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    children: [
                      ...
                      if (_filteredBookings.isEmpty)
                        const Padding(
                          ...
                          child: Center(
                            child: Text(
                              'لا توجد حجوزات — اضغط "حجز جديد" للبدء',
                              style: TextStyle(color: AppColors.textMuted),
                            ),
                          ),
                        )
